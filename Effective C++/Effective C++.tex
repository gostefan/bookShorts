...
As some API function might rely on the raw resource rather than the resource managing classes you need access to the raw resource.

If the resource has member functions of its own it might be reasonable to define \code{operator->}.

Finally if you want implicit conversion of the managing type to the resource type you want to define \code{operator Container() const}. But this might also lead to unwanted conversions.
