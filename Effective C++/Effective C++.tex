...
Sometimes it's better to (log and) swallow an exception than to have/run the risk of premature program termination.
\paragraph{}
An alternative approach would be to extract the functionality in the destructor to a funciton - e.g. \code{close} that may throw. That way the client can handle the exception. If on object destruction that function was not called already call it and use one of the above approaches to handle the exception.\\
Be aware how this solution relates to the RAII idiom.0
